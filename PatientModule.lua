...
function PatientModule.Init(State, Toggles)
    ...
    self.ROOM_SIZE = Vector3.new(45, 45, 45)
...
function PatientModule:Start()
    ...
    self.TaskThread = task.spawn(function()
        ...
        local nextBottomIsB = true

        while self.Running do
            self:DPrint("Checking Top A for patients...")
            ...
            if nextBottomIsB then
                self:DPrint("Cycling to Bottom B...")
                self:ProcessLowerRoom("Bottom B", self.LOWER_ROOM_B_CFRAME, hrp, hum)
                nextBottomIsB = false
            else
                self:DPrint("Cycling to Bottom C...")
